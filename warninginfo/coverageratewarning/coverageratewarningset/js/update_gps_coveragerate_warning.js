 ...
 * @desc 修改数据-保存
 */
function updateCoveragerateWarning(){
	disableButtion("saveButton");
	var bool=$('#gpsCoveragerateWarningForm').form('validate');
	if(bool==false){
		enableButtion("saveButton");
		return bool;	
	}
	$.ajax({
		url : rootPath+"/gpscoverageratewarning/update.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#gpsCoveragerateWarningForm').serializeToJson()),
		success: function(data){
			if(data.status==1){
				businessId=data.data;
				top.showAlert("提示", "更新成功", 'info', function() {
					reloadData("query_gps_coveragerate_warning.html","#gpsCoveragerateWarningdatagrid");
					closePanel();
				});
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsCoveragerateWarning");
}
