 ...
 * @desc 修改数据-保存
 */
function updateGpsDevOfflineWarning(){
	disableButtion("saveButton");
	var bool=$('#gpsDevOfflineWarningForm').form('validate');
	if(bool==false){
		enableButtion("saveButton");
		return bool;	
	}
	$.ajax({
		url : rootPath+"/gpsdevofflinewarning/update.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#gpsDevOfflineWarningForm').serializeToJson()),
		success: function(data){
			if(data.status==1){
				businessId=data.data;
				top.showAlert("提示", "更新成功", 'info', function() {
					reloadData("query_gps_dev_offline_warning.html","#gpsDevOfflineWarningdatagrid");
					closePanel();
				});
...
function loadData(jsondata){
	...
	$('#deadlinetime').val(jsondata.deadlinetime);
}
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsDevOfflineWarning");
}
