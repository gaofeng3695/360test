...
function updateProjectProgress(){
	...
	$.ajax({
		url : rootPath+"/projectprogress/update.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#projectProgressForm').serializeToJson()),
 ...
 * @desc 获得数据
 */
function getProjectProgressById(){
	$.ajax({
		url : rootPath+"/projectprogress/get.do",
		data :{"oid" : pkfield},
		type : 'POST',
		dataType:"json",
		async:true,
...
function loadData(jsondata){
	...
	$('#progressdes').val(jsondata.progressdes);
	$('#gpsConstructionOid').val(gpsConstructionOid);
}
 ...
 */
function closePanel() {
	top. closeDlg("updateProjectProgress");
}
