...
*/});
function getGpsConstructionClose(){
	$.ajax({
		url : rootPath+"/gpsconstructionclose/get.do",
		data :{"oid" : pkfield},
		type : 'POST',
		dataType:"json",
		async:true,
		success : function(data) {
			if(data.status==1){
				...
				// 加载图片信息
				getPicListInfo(cid, "view");  // 不带描述信息  图片
				getFileListInfo(cid, "view"); // 附件信息
				masterTableChildHeight()	// 计算标签内容高度
				masterTableChildHeight1('changeinfo')	// 计算标签内容高度
			}else{
 ...
 * @desc 修改数据-保存
 */
function updateGpscloseConstruction(){
	...
	$.ajax({
		url : rootPath+"/gpsconstructionclose/updateCloseInfo.do",
		type: "post",
...
function loadCloseData(jsondata){
	$('#coid').val(jsondata.coid);
	$('#oid').val(jsondata.oid);
	$("#closedescr").val(jsondata.closedescr);
	$("#closedate").val(jsondata.closedate);
}
 ...
 */
function closePanel() {
	top. closeDlg("updateCloseGpsConstruction");
}
