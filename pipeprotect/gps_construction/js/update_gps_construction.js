...
function getMarker(lineloopid, stakenum){
	$('#stakenum').combobox({
		panelHeight:150,
		editable:true,
		url : rootPath+'/gpsmarker/querySelectByLineloopAndType.do?lineloopid='+lineloopid+'&unitid='+$('#unitid').combotree('getValue')+'&markerOid='+stakenum,
		valueField : "codeid",
		textField : "codename",
 ...
 * @desc 修改数据-保存
 */
function updateGpsConstruction(){
	...
	$.ajax({
		url : rootPath+"/gpsconstruction/update.do",
		type: "post",
...
function getGpsConstructionById(){
	$.ajax({
		url : rootPath+"/gpsconstruction/get.do",
		data :{"oid" : pkfield},
		type : 'POST',
		dataType:"json",
		async:false,
...
function loadData(jsondata){
	...
	$('#stakenum').combobox('setValue',jsondata.stakenum);
	...
	$('#constructreason').val(jsondata.constructreason);
	...
	$('#isProhibitedItems').combobox('setValue',jsondata.isProhibitedItems);
	setComboObjWidth('isProhibitedItems',0.270,'combobox');
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsConstruction");
}
