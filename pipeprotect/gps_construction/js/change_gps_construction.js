 ...
 * @desc 修改数据-保存
 */
function saveGpslevelchangeConstruction(flag){
	...
	$.ajax({
		url : rootPath+"/gpsconstructionlevelchange/saveChangeInfo.do",
		type: "post",
 ...
 */
function closePanel() {
	top. closeDlg("changeGpsConstruction");
}
...
function submit() {
	...
	var subject="第三方施工:"+$('#projectname').text()+"风险等级变更,变更前:"+$('#riskratingCodeName').text()+",变更后:"+$('#changeriskrating').combobox('getText');
