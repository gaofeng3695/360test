...
var pkfield=getParamter("oid");	// 业务数据closeID
//当前登陆用户
 ...
 * @desc 获得数据
 */
function getGpsEventById(eventoid){
	$.ajax({
	...
	$.ajax({
		...
		success : function(data) {
			if(data.status==1){
				...
				getGpsEventById(jsondata.eventoid);
				getGpsHarmEventById(jsondata.eventoid);
				getGpsDestroyEventById(jsondata.eventoid);
 ...
 * @returns
 */
function getGpsHarmEventById(eventoid){
	$.ajax({
		...
		success : function(data) {
			if(data.status==1){
				...
				if(jsondata != null && jsondata.length != 0){
					var rs = '<div id="harmdiv" icon="icon-reload"><div id="contentArea" class="content-area"><div class="content-area-main">';
						...
						rs  += '<div class="table-content">\
							...
							<table align="center" class="event-detail-table" >\
								<tr>\
									...
									<td width="85%"><span>'+(jsondata[i].harmdate==null?'':jsondata[i].harmdate)+'</span></td>\
								</tr>\
								<tr>\
 ...
 * @returns
 */
function getGpsDestroyEventById(eventoid){
	$.ajax({
		...
		success : function(data) {
			if(data.status==1){
				var jsondata = data.data;
				if(jsondata != null && jsondata.length != 0){
					var rs = '<div id="destroydiv" icon="icon-reload"><div id="contentArea" class="content-area"><div class="content-area-main">';
					var j = jsondata.length;
					for(var i=0; i<jsondata.length;i++){
						rs  += '<div class="table-content">\
 ...
 */
function closePanel() {
	top. closeDlg("viewCloseGpsEvent");
}
