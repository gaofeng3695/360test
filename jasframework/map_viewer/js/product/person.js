...
var person = Vue.component('person', {
    ...
    mounted: function mounted() {
        ...
        person.personVm.$options.methods.getAllPersonOnlineOrOff(person.personVm.departmentOidProp, person.personVm.hierarchyProp); // 添加缓冲区图层
    ...
    methods: {
        ...
        getAllPersonOnlineOrOff: function getAllPersonOnlineOrOff(unitId, hierarchy) {
            /* 获取本部门的总人数，在线人数，离线人数。 */
            Vue.prototype.$http.post(person.personVm.rootPath + '/realtimemonitor/realtimemonitor/getInspectorCountByUnitid.do?unitid=' + unitId + '&token=' + person.personVm.token + '&hierarchy=' + hierarchy).then(function (response) {
        ...
        getParentDeparement: function getParentDeparement(departmentOid, parentId) {
            Vue.prototype.$http.post(person.personVm.rootPath + '/realtimemonitor/realtimemonitor/getParentUnitList.do?unitId=' + departmentOid + '&token=' + person.personVm.token + '&parentId=' + parentId).then(function (response) {
                /* 设置列表部门。 */
         ...
         */
        getPerson: function getPerson(online, hierarchy, inspectorType, pageNo, inspectorName, date) {
            person.personVm.onlineProp = online;
            /* 获取部门下的人员。 */

            Vue.prototype.$http.post(person.personVm.rootPath + '/realtimemonitor/realtimemonitor/getEmployeeByUnitId.do?token=' + person.personVm.token + '&hierarchy=' + hierarchy + "&online=" + online + "&inspectorType=" + inspectorType + "&date=" + person.personVm.date + "&pageNo=" + pageNo, {
                "inspectorName": person.personVm.inspectorName
            }, {
                headers: {
         ...
         * @returns
         */
        getInspectorWeekScore: function getInspectorWeekScore(inspectorId, inspectorName) {
         ...
         * 定位所有人员
         */
        positionPerson: function positionPerson() {
            /* 先清理所有地图上已经展示的图层。 */
            ...
            for (var i = 0; i < person.personVm.personList.length; i++) {
                /* 在线并且经纬度不为NULL才能定位。 */
                if (person.personVm.personList[i].lon != null && person.personVm.personList[i].lon != '' && person.personVm.personList[i].lat != null && person.personVm.personList[i].lat != '' && person.personVm.personList[i].status == 1) {
                    var array = [];
                    var position = {};
                    ...
                    array.push(position);
                    var insname = person.personVm.personList[i].insname;
                    /* 如果是管道工 */
                    ...
                    option.attributes = {
                        "oid": position.oid,
                        "date": person.personVm.personList[i].locationdate,
                        "departmentOid": person.personVm.departmentOidProp,
                        "hierarchy": person.personVm.hierarchyProp,
                        "insname": insname
                    };
                    ...
                    _options.fontFamily = "微软雅黑";
                    /*options.haloColor = [0,0,0, 130];
                    options.haloSize= 6;*/
                    // options.color= [255,255,255,255];

                    _options.color = [41, 52, 94, 255];
                    _options.background = [0, 0, 0, 130];
                    _options.angle = 0; //options.border="1px solid #d3d3d3";

                    _options.haloColor = [255, 255, 255, 130];
                    _options.haloSize = 1;
                    _options.backgroundColor = [0, 0, 0, 130];
                    _options.fontWeight = "bold";
                    _options.angle = 0;
                    _options.xOffset = -55;
                    _options.yOffset = -30;
                    _options.center = false;
                    _options.layerId = "PERSON_TEXT";
                    /* 如果是巡线工 */

                    if (person.personVm.personList[i].inspectortype == '01') person.personVm.jasMapApi.addTextGraphic(person.personVm.personList[i].lon, person.personVm.personList[i].lat, person.personVm.personList[i].insname + person.personVm.personList[i].locationdate, _options);else person.personVm.jasMapApi.addTextGraphic(person.personVm.personList[i].lon, person.personVm.personList[i].lat, person.personVm.personList[i].insnamePlumber + person.personVm.personList[i].locationdate, _options);
                }
            }
            /* 点击相应的人员到达对应的人员详情页面 */
         ...
         * 巡线工定位
         */
        locateInspector: function locateInspector(inspectorOid, lon, lat, name, status) {
            if (status == 0) {
        ...
        clearAllLayer: function clearAllLayer() {
            for (var i = 0; i < person.personVm.allLayer.length; i++) {
         ...
         * @returns
         */
        getInspectorKeyPoint: function getInspectorKeyPoint(inspectorOid) {
            /* 获取部门下的所有部门或者人员。 */
            Vue.prototype.$http.post(person.personVm.rootPath + '/realtimemonitor/realtimemonitor/getInsKeyPointByInspectorid.do?inspectorid=' + inspectorOid + '&token=' + person.personVm.token + '&date=' + person.personVm.date).then(function (response) {
                var result = response.data;
                var bufferOptions = {}; //清除上一次添加的关键点图层
                ...
                if (result && result.data.length > 0) {
                    ...
                    person.personVm.jasMapApi.setLevel(14); //添加关键点图层

                    var _options2 = {
                        "id": "PERSON_KEYPOINT",
                        "type": "graphic"
                    };
                    person.personVm.jasMapApi.addLayer(_options2);
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (data[i].keypointtype == 1) {
                            ...
                            if (data[i].pointstatus == 0) {
                                img = "images/keypoint_flag_red.png"; // 关键点缓冲区
                                ...
                                img = "images/keypoint_flag_green.png"; // 关键点缓冲区
                        ...
                        person.personVm.jasMapApi.addPictureGraphic(data[i].lon, data[i].lat, _options3);
                        /* 添加点击事件 */
                        ...
                        person.personVm.jasMapApi.addLayerClickEventListener("PERSON_KEYPOINT", function (e) {
                            ...
                            if (type == 1) {
                                person.personVm.otherJs.showDialog2('viewGpsKeypoint', '关键点详细', '../../realtimemonitor/realtimemonitor/gpskeypoint.html?oid=' + oid, 800, 600, false, ''); //queryTop(oid,"GPS_KEYPOINT","../../linePatrolManage/pathLine/keypoint/view_gps_keypoint.html?oid=","viewGpsKeypoint","详细");
                            } else if (type == 2) {
                                person.personVm.otherJs.showDialog2('viewGpsTemporaryKeypoint', '关键点详细', '../../realtimemonitor/realtimemonitor/gpstemporarykeypoint.html?oid=' + oid, 800, 600, false, ''); //queryTop(oid,"gps_temporary_keypoint","../../linePatrolManage/insTask/temporaryKeypoint/view_gps_temporary_keypoint.html?oid=","viewGpsTemporaryKeypoint","详细");
                            }
                        });
                        _options3 = {};
                        _options3.fontSize = 12;
                        _options3.fontFamily = "微软雅黑";
                        /*options.haloColor = [0,0,0, 130];
                        options.haloSize= 6;*/
                        // options.color= [255,255,255,255];

                        _options3.color = [41, 52, 94, 255];
                        _options3.background = [0, 0, 0, 130];
                        _options3.angle = 0; //options.border="1px solid #d3d3d3";

                        _options3.haloColor = [255, 255, 255, 130];
                        _options3.haloSize = 1;
                        _options3.backgroundColor = [0, 0, 0, 130];
                        _options3.fontWeight = "bold";
                        _options3.angle = 0;
                        _options3.xOffset = -20;
                        _options3.yOffset = -30;
                        _options3.center = false;
                        _options3.layerId = "COMPANY_TEXT";
                        person.personVm.jasMapApi.addTextGraphic(data[i].lon, data[i].lat, data[i].pointidname, _options3);
                    }
                    /* 展示完毕后，将已经加载的图层加入到 allLayer 中 */
         ...
         * 通过人员id获取到该人员当日的轨迹
         */
        getLineByDateInspectorOid: function getLineByDateInspectorOid(inspectorOid, inspectorName) {
            Vue.prototype.$http.post(person.personVm.rootPath + '/realtimemonitor/realtimemonitor/getInsLineByDateInspectorid.do?inspectorid=' + inspectorOid + '&token=' + person.personVm.token).then(function (response) {
                var data = response.data;

                if (data.status == 1) {
                    //清除上一次加载的轨迹起始终止点图层
                    jasMapApi.clearGraphicsLayer("drawlayer_MONITORABPOINT"); //清除上一次加载的当日巡线轨迹图层
                    ...
                    if (data && data.data.data.length > 0) {
                        ...
                        if (!jasMapApi) {
                            ...
                            jasMapApi.addLayer(options); // 添加回放时加载的起始终止点标绘

                            options = {
                                url: "../../realtimemonitor/playback/images/inslocation_A.png",
                                //图标地址
                                width: 36,
                                //图标大小
                                height: 36,
                                attributes: {
                                    "OBJECTID": pointdata[0].oid,
                                    "TITLE": "轨迹起始点"
                                },
                                //图标属性
                            ...
                            options = {
                                url: "../../realtimemonitor/playback/images/inslocation_B.png",
                                //图标地址
                                width: 36,
                                //图标大小
                                height: 36,
                            ...
                            jasMapApi.addLayer(options); // 添加当日轨迹标绘
         ...
         * 展示人员详细信息
         */
        showPersonDetail: function showPersonDetail(unitId, hierarchy, inspectorOid, insname) {
            /* 将组件内容切换为人员详情 */
            if (person != undefined && person != '' && person != null && person.personVm._isDestroyed) {
                if (detail != null && detail != undefined && detail != '' && detail.detailVm != null && detail.detailVm != undefined && detail.detailVm != '') {
