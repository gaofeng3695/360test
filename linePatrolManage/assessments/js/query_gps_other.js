...
function initOneDatagrid(url, hierarchy){
    if( url == undefined || url == null || url == '' ) {
        url = rootPath+"/other/unit/getPage.do?parentId="+user.unitId+"&statisticsDate="+$('#statisticsdate').val();
    }
...
function enterContent( unitId, hierarchy, unitName ) {
    ...
    if(hierarchy.split('.').length <= 3) {
        url = rootPath+"/other/unit/getPage.do?parentId="+unitId+"&statisticsDate="+$('#statisticsdate').val();
        initOneDatagrid( url, hierarchy );
        /* 修改展示界面 */
        currentShowInterface = "unit";
    } else {
        /* 根据部门查询是否有子部门，如果没有子部门，就显示段长列表。 */
        url = rootPath+"/other/getPage.do?unitid="+unitId+"&statisticsDate="+$('#statisticsdate').val();
        initInsDatagrid( url );
...
function initInsDatagrid( url ){
    ...
    if( url == undefined || url == null || url == '' ) {
        url = rootPath+"/other/getPage.do?unitid="+user.unitId+"&statisticsDate="+$('#statisticsdate').val();
    }
...
function exportQuery(){
    ...
    if(currentShowInterface == "segmentHead"){
        url=addTokenForUrl(rootPath+'/other/exportToExcelAction.do?unitid='+currentUnit.unitId+'&statisticsDate='+$('#statisticsdate').val());
    }else if(currentShowInterface == "unit"){
        url=addTokenForUrl(rootPath+'/other/unit/exportToExcelAction.do?parentId='+currentUnit.unitId+'&statisticsDate='+$('#statisticsdate').val());
    }
