...
$(document).ready(function(){
	$('#printBtn').on('click', function() {
		$('#changeinfo .table-content #description').printThis({
			header: "<h1 style='text-align: center'>区长岗位描述</h1>"
		});
	})
	getGpsInsrangeById();
	initKeyPoingData();
...
function loadData(jsondata){
    ...
    $("#highConsequencesLevelName").html(jsondata.highConsequencesLevelName);
    $("#highConsequencesTypeName").html(jsondata.highConsequencesTypeName);
	...
	$('#designAreaGradeCodeName').html(jsondata.designAreaGradeCodeName);
	$('#districtGradeCodeName').html(jsondata.districtGradeCodeName);
	$('#controlMeasures').html(jsondata.controlMeasures);
	$('#identificationItem').html(jsondata.identificationItem);
	$('#lethalRadiusArea').html(jsondata.lethalRadiusArea);
	$('#potentialImpactArea').html(jsondata.potentialImpactArea);
	$('#evaluationTime').html(jsondata.evaluationTime);
	$('#lastIdentificationTime').html(jsondata.lastIdentificationTime);
