...
//	loadMoreSelectData(morecomboxid,moreDomainName);
	...
	setComboObjWidth('execunitid',0.281,'combobox');
	getGpsBufferWidthById();
});

/**
 * @desc 修改数据-保存
 */
function updateGpsBufferWidth(){
	...
	$.ajax({
		url : rootPath+"/gpsbufferwidth/update.do",
		type: "post",
		async: false,
 ...
 * @desc 获得数据
 */
function getGpsBufferWidthById(){
	$.ajax({
		url : rootPath+"/gpsbufferwidth/get.do",
		data :{"oid" : pkfield},
		type : 'POST',
		dataType:"json",
		async:true,
...
function loadData(jsondata){
	...
	$('#bufferwidth').val(jsondata.bufferwidth);
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsBufferWidth");
}
