...
$(document).ready(function() {
	...
	$('#insfreq').combobox({
		...
		onClick : function(row){
			...
			if(m == 1){
				...
				top.getDlg("select_temporary_point_time.html?original=update&&count="+n+"&type=single"+"&mn="+mn,"selectTemporaryPointTime","选择关键点执行时间",800,300,false,true,true);
			}else{
				/* 如果不是一天N巡，就设置字段值为'' */
				/*$('#taskTime').val('');
				sessionStorage.removeItem('timeArray');*/
				...
				top.getDlg("select_temporary_point_time.html?original=update&count="+n+"&type=multiple"+"&mn="+mn,"selectTemporaryPointTime","选择关键点执行时间",800,300,false,true,true);
			}
	...
	setComboObjWidth("llineloopoid",0.275,'combobox');
	...
	setComboObjWidth("lpointtype",0.275,'combobox');
	...
	setComboObjWidth("lmarkeroid",0.275,'combobox');
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsTemporaryKeypoint");
}
...
function addLocation(){
    /* 打开地图 */
    top.map.show = true;
    top.hideDlg('updateGpsTemporaryKeypoint');
...
function getLocationCallBack(obj){
		...
		$('#lon').val(x);
		$('#lat').val(y);
	    top.showDlg("updateGpsTemporaryKeypoint");
