...
$(document).ready(function() {
	...
	/* 关键点类型默认设置为临时关键点 */
	$('#pointtype').combobox('setValue', '01');
	...
	setComboObjWidth("inspectorid",0.280,'combobox');*/
	...
	$('#insfreq').combobox({
		...
		onClick : function(row){
			...
			if(m == 1){
				...
				top.getDlg(rootPath+"/linePatrolManage/insTask/temporaryKeypoint/select_temporary_point_time.html?original=save&&count="+n+"&type=single"+"&mn="+mn,"selectTemporaryPointTime","选择关键点执行时间",800,300,false,true,true);
			}else{
				/* 如果不是一天N巡，就设置字段值为'' */
				/*$('#taskTime').val('');
				sessionStorage.removeItem('timeArray');*/
				...
				top.getDlg("select_temporary_point_time.html?original=save&count="+n+"&type=multiple"+"&mn="+mn,"selectTemporaryPointTime","选择关键点执行时间",800,300,false,true,true);
			}
			
		}
	});
	setComboObjWidth("insfreq",0.280,'combobox');
	...
	setComboObjWidth("confirmorid",0.280,'combobox');
	...
	$('#llineloopoid').combobox({
		...
		onSelect : function(row){
			...
			if(type != ''){
				...
				/* 发送请求，获取桩号 */
				$('#lmarkeroid').combobox("reload",rootPath+'/gpsmarker/querySelectByLineloopAndType.do?markertype='+type+'&lineloopid='+row.codeid+'&unitid='+$('#unitid').combotree('getValue'));
			}
		}
	});
	setComboObjWidth("llineloopoid",0.280,'combobox');
	...
	setComboObjWidth("lpointtype",0.280,'combobox');
	...
	setComboObjWidth("lmarkeroid",0.280,'combobox');
 ...
 */
function closePanel() {
	top. closeDlg("addGpsTemporaryKeypoint");
}
...
function loadSelectData(comboxid,singleDomainName){
	if(comboxid!='' && comboxid !=undefined){
		...
		for(var i=0;i<select.length-1;i++){
			...
			setComboObjWidth(id,0.280,'combobox');
		}
	}
}
...
/* 添加位置坐标 */
	...
	/* 打开地图 */
    top.map.show = true;
    top.hideDlg('addGpsTemporaryKeypoint');
...
function getLocationCallBack(obj){
		...
		$('#lon').val(x);
		$('#lat').val(y);
	    top.showDlg("addGpsTemporaryKeypoint");
