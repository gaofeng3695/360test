...
$(document).ready(function(){
	...
	for( let i = 0 ; i < times ; i++ ){
		$('#startTime'+(i+1)).on('click',function(){
			WdatePicker({dateFmt:'HH:mm:ss',minTime:'06:00',maxTime:'20:00'});
		})
 ...
 */
function closePanel() {
	top.closeDlg("selectTemporaryPointTimeconstruct");
}
...
function reloadData(shortUrl) {
	...
	for ( var i = 0; i < fra.length; i++) {
		if (fra[i].id=='iframe_addGpsTemporaryKeypointconstruct' || fra[i].id=='iframe_updateGpsTemporaryKeypointconstruct') {
