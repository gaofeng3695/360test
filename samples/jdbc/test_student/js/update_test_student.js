...
function updateSuccessFun(){
	top.showAlert("提示", "更新成功", 'info', function() {
		//关闭弹出框
		reloadData("query_test_student.html","#testStudentdatagrid");
	    closePanel();
	});
}
/**
 * @desc 修改数据-保存
 */
function updateTestStudent(){
	disableButtion("saveButton");
	var bool=$('#testStudentForm').form('validate');
	if(bool==false){
		enableButtion("saveButton");
		return bool;	
	}
	$.ajax({
		url : rootPath+"/teststudent/update.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#testStudentForm').serializeToJson()),
...
function loadData(jsondata){
	...
	$('#studentCode').val(jsondata.studentCode);
 ...
 */
function closePanel() {
	top. closeDlg("updateTestStudent");
}
