...
function updateSuccessFun(){
	top.showAlert("提示", "更新成功", 'info', function() {
		//关闭弹出框
		reloadData("query_test_cross.html","#testCrossdatagrid");
	    closePanel();
	});
}
/**
 * @desc 修改数据-保存
 */
function updateTestCross(){
	disableButtion("saveButton");
	var bool=$('#testCrossForm').form('validate');
	if(bool==false){
		enableButtion("saveButton");
		return bool;	
	}
	$.ajax({
		url : rootPath+"/testcross/update.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#testCrossForm').serializeToJson()),
...
function loadData(jsondata){
	...
	$('#crossCode').val(jsondata.crossCode);
	$('#startStakeOid').val(jsondata.startStakeOid);
	$('#startRelativeMileage').val(jsondata.startRelativeMileage);
	$('#endStakeOid').val(jsondata.endStakeOid);
	$('#endRelativeMileage').val(jsondata.endRelativeMileage);
	$('#projectOid').val(jsondata.projectOid);
	$('#crossType').val(jsondata.crossType);
	$('#shape').val(jsondata.shape);
	$('#shapeState').val(jsondata.shapeState);
}
