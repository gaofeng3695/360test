...
$(document).ready(function(){
	$('#11020106').menubutton({menu:'#exportBars'});
	
	if(flag==null || flag == '')
		flag = '';
	$('#gpsDevicedatagrid').datagrid({
		idField:'oid',
		url: rootPath+"/gpsdevice/getPage.do?flag="+flag+"&inspectorType=01",
		collapsible:true,
		autoRowHeight: false,
		remoteSort:true,
        fitColumns: true,
		...
		[[
	    	...
	    	},
			{
				...
				formatter: function(value,row,index){
					if(row.inspectortype == '02') {
						return '（段长/区长）'+row.userName;
					} else if( row.inspectortype == '01' ) {
						return '（巡线工）'+row.insname;
					} else {
						return '';
					}
				}
		  	...
		  	/*{
		  		...
		  		formatter: function(value,row,index){
		  			...
		  			if(row.unitid == user.unitId){
		  				if(row.approvalStatus == '01' || row.approvalStatus == '02' || row.approvalStatus == '03'){
		  					var opt = '<p class="table-operate"><a class = "a11020101" href="#" title = "查看" onclick="view(\'' + row.oid+'\')">\
		  								<span class="fa fa-eye"></span>\
									   </a><a class = "a11020103" href="#" title = "修改" onclick="update(\'' + row.oid+'\')">\
							...
							return opt;
			  				...
			  				var opt = '<p class="table-operate"><a class = "a11020101" href="#" title = "查看" onclick="view(\'' + row.oid+'\')">\
											...
											<span class="fa fa-edit"></span>\
								   	   </a><a class = "a11020104" href="#" title = "删除" onclick="dele(\'' + row.oid+'\')">\
 ...
 */
function importData(){
	 var funcName = "巡检设备管理模板";	// 导入模板功能名称
	 top.getDlg(rootPath+"jasframework/components/excel/importExcelData.htm?tableName=gps_device&callerPageUrl=query_gps_device.html&datagridElementId=gpsDevicedatagrid&functionName="+encodeURI(encodeURI(funcName)),"importiframe","导入",700,410);
}
...
function exportSelect(){
	...
	if(!isNull(idArray)){
		 url=addTokenForUrl(rootPath+'/gpsdevice/exportToExcelAction.do?oidList='+idArray);
...
function exportQuery(){
	var url =addTokenForUrl(rootPath+'/gpsdevice/exportToExcelAction.do?'+$('#queryForm').serialize());
	$("#exprotExcelIframe").attr("src",url);
}
...
function check(){
	...
	var bool = true;
	if (rows.length == 0){
		$("#11020103").linkbutton('enable');
		$("#11020104").linkbutton('enable');
		$("#11020108").linkbutton('enable');
		return;
	}else{
	...
	if(bool){
		if(rows[0].approvalStatus == '01' || rows[0].approvalStatus == '02'  ){
			$("#11020108").linkbutton('disable');
			$("#11020104").linkbutton('disable');
			$("#11020103").linkbutton('disable');
		}else{
			$("#11020104").linkbutton('enable');
			$("#11020103").linkbutton('enable');
			$("#11020108").linkbutton('enable');
