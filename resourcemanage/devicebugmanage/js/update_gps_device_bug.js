...
$(document).ready(function() {
	...
	setComboObjWidth("approvaple",0.270,'combobox');
	
	/* 初始化combobox */
	$('#approvaple').combobox('loadData', userArray);
	/* 初始化值 */
	$('#approvaple').combobox('setValue', user.oid);
	
/*	getPicListInfo(businessId, "view");  // 不带描述信息  图片
	getFileListInfo(businessId, "view"); // 附件信息
*/	
...
function updateSuccessFun(){
	top.showAlert("提示", "审核成功", 'info', function() {
		//关闭弹出框
...
function updateGpsDeviceBug(){
	...
	$.ajax({
		url : rootPath+"/gpsdevicebug/update.do",
		type: "post",
		async: false,
...
function rejectGpsDeviceBug(){
	...
	$.ajax({
		url : rootPath+"/gpsdevicebug/reject.do",
		type: "post",
		contentType: "application/json;charset=utf-8",
		dataType: "json",
		data:JSON.stringify($('#gpsDeviceBugForm').serializeToJson()),
		success: function(data){
			if(data.status==1){
				businessId=data.data;
				/*updateFileFun(updateFileSuccessFun);*/  // 先上传附件 后上传图片
				top.showAlert("提示", "驳回成功", 'info', function() {
					//关闭弹出框
					reloadData("query_gps_device.html","#gpsDevicedatagrid");
				    closePanel();
				});
			}else if(data.code == "400") {
				top.showAlert("提示", "驳回失败", 'error');
				enableButtion("saveButton");
			}else{
				top.showAlert("提示", data.msg, 'info');
				enableButtion("saveButton");
 ...
 */
function closePanel() {
	top. closeDlg("updateGpsDeviceBug");
}
